...
# ==============================================================================
r"""Convert road sign dataset to TFRecord.

Example usage:
    python create_roadsign_tfrecord.py  --data_dir=/tmp/raw_data \
        --output_path=/tmp/tfrecord
"""
...
pascal_label_map_dict = {
    'background': 0, 'trafficlight': 1, 'stop': 2, 'speedlimit': 3, 'crosswalk': 4
}
...
def dict_to_tf_example(data,
  ...
  example = tf.train.Example(features=tf.train.Features(feature={
      ...
      'image/encoded': tfrecord_util.bytes_feature(encoded_jpg),#img_raw),#
      'image/format': tfrecord_util.bytes_feature('png'.encode('utf8')),#'jpeg'.encode('utf8')),
...
def main(_):
  ...
  logging.info('Reading from road sign dataset.')

  img_list = os.listdir(os.path.join(data_dir, 'images'))
  X_train, X_test = train_test_split(img_list, test_size=0.1, train_size=0.9, random_state=123)

  annotations_dir = os.path.join(data_dir, 'annotations')

  if FLAGS.set == 'train':
    for idx, x_file_name in enumerate(X_train):
      if FLAGS.num_images and idx >= FLAGS.num_images:
        break
      if idx % 100 == 0:
        logging.info('On image %d of %d', idx, len(X_train))
      file_name_stem = Path(x_file_name).stem
      ann_file_path = os.path.join(annotations_dir, file_name_stem + '.xml')
      with tf.gfile.GFile(ann_file_path, 'r') as fid:
        xml_str = fid.read()
    ...
    for idx, x_file_name in enumerate(X_test):
      if FLAGS.num_images and idx >= FLAGS.num_images:
        break
      if idx % 50 == 0:
        logging.info('On image %d of %d', idx, len(X_train))
