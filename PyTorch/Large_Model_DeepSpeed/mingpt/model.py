...
class GPT(nn.Module):
    ...
    def get_default_config():
        ...
        C.resid_pdrop = 0.1
        C.attn_pdrop = 0.1
        C.activation_checkpoint = False
        C.checkpoint_num_layers = 1
        return C
    ...
    def __init__(self, config):
        ...
        if self.activation_checkpoint:
            ...
            self.checkpoint_fn = deepspeed.checkpointing.checkpoint
    ...
    def forward(self, idx, targets=None):
        def custom(start, end):
            def custom_forward(input):
                layers_ = self.transformer.h[start:end]
                x_ = input
        ...
        if self.activation_checkpoint:
            ...
            while layer < num_layers:
                x = self.checkpoint_fn(custom(layer, layer + chunk_length), x)
                layer += chunk_length
        else:
